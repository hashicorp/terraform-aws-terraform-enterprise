...
module "service_accounts" {
  ...
  database_passwordless_aws_use_iam     = var.database_passwordless_aws_use_iam
...
module "runtime_container_engine_config" {
  source = "git::https://github.com/hashicorp/terraform-random-tfe-utility//modules/runtime_container_engine_config?ref=pravi/IND-5776-release-test"
  ...
  # Use placeholder password for IAM auth (TFE config validation requires it), otherwise use the database password
  database_password         = local.database_passwordless_aws_use_iam ? "aws-iam-auth" : local.database.password
...
# --------------------------------------------------------------------------------------------------
module "tfe_init" {
  source = "git::https://github.com/hashicorp/terraform-random-tfe-utility//modules/tfe_init?ref=pravi/IND-5776-release-test"
  count  = var.is_replicated_deployment ? 0 : 1
...
# --------------------------------------------------------------------------------------------
module "settings" {
  source = "git::https://github.com/hashicorp/terraform-random-tfe-utility//modules/settings?ref=pravi/IND-5776-release-test"
...
module "tfe_init_replicated" {
  source = "git::https://github.com/hashicorp/terraform-random-tfe-utility//modules/tfe_init_replicated?ref=pravi/IND-5776-release-test"
  count  = var.is_replicated_deployment ? 1 : 0
